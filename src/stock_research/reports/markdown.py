...
def _annual_facts_table(state: ResearchState) -> str:
    ...
    return "\n".join(lines) + "\n\nAmounts are in RMB billions unless otherwise noted. Full annual history lives in the data-linkage appendix."
...
def _quarterly_facts_table(state: ResearchState) -> str:
    ...
    return (
        "\n".join(lines)
        + "\n\nAmounts are in RMB billions except diluted shares, which are in billions of ordinary shares. Full quarterly history lives in the data-linkage appendix."
    )
...
def _metric_pack_compact_summary(metrics: list[dict[str, Any]]) -> str:
    if not metrics:
        return "- No metrics calculated yet."
    calculated = []
    review_required = []
    ...
    for metric in metrics:
        ...
        if formula_id == "financial_quality_questions_v1":
            continue
        if formula_id == "latest_interim_trend_v1":
            latest_interim = metric
            continue
        annual_results = metric.get("annual_results") or []
        has_calculated = any(result.get("status") == "calculated" for result in annual_results)
        if has_calculated:
            calculated.append(formula_id)
        else:
            not_calculated.append(formula_id)
        if any(
            result.get("review_required") or result.get("limitations") or result.get("assumption")
            for result in annual_results
        ):
            review_required.append(formula_id)
    lines = [
        f"- Financial metric families calculated or attempted: {len([m for m in metrics if m.get('formula_id') != 'financial_quality_questions_v1'])}.",
        f"- Calculated annual metric families: {len(calculated)}.",
    ]
    if latest_interim:
        lines.append(
            f"- Latest-interim trend: {latest_interim.get('overall_status', latest_interim.get('status'))}; "
            f"latest period {latest_interim.get('latest_period_end', 'n/a')}."
        )
    if review_required:
        lines.append("- Review-required formula families: " + ", ".join(review_required[:8]) + ".")
    if not_calculated:
        lines.append("- Not fully calculated: " + ", ".join(not_calculated[:8]) + ".")
    lines.append("- Full formula outputs live in `financial_report_pack.json` and the data-linkage appendix.")
    return "\n".join(lines)
...
def _financial_quality_questions_compact(
    state_or_metrics: ResearchState | list[dict[str, Any]],
    metrics: list[dict[str, Any]] | None = None,
) -> str:
    if isinstance(state_or_metrics, dict):
    ...
    lines = [
        "This table is the consolidated diagnostic view. Detailed explanations stay in the Chinese easy-reading report and the financial report pack.",
    ]
    if diagnostics:
        summary = diagnostics.get("summary", {})
        lines.append(
            f"- Diagnostic status: {diagnostics.get('status')} | answered {summary.get('answered', 0)}, "
            f"partial {summary.get('partial', 0)}, missing {summary.get('missing', 0)}."
        )
    lines.extend(
        [
            "",
            "| # | Question | Status | Key read | Watch item / gap |",
            "| ---: | --- | --- | --- | --- |",
        ]
    )
    for question in questions:
        values = question.get("latest_values") or {}
        key_read = _compact_question_key_read(str(question.get("question_id") or ""), values)
        watch_items = []
        warnings = question.get("warning_flags") or []
        missing = question.get("missing") or []
        if warnings:
            watch_items.append("; ".join(_warning_label(warning) for warning in warnings[:2]))
        if missing:
            watch_items.append("Missing: " + ", ".join(str(item) for item in missing[:3]))
        lines.append(
            "| {rank} | {question} | {status} | {key_read} | {watch} |".format(
                rank=question.get("rank", ""),
                question=_markdown_cell(question.get("question") or question.get("question_id")),
                status=_markdown_cell(question.get("status")),
                key_read=_markdown_cell(key_read),
                watch=_markdown_cell("; ".join(watch_items) if watch_items else "No major V1 gap flagged"),
            )
        )
    return "\n".join(lines)


def _compact_question_key_read(question_id: str, values: dict[str, Any]) -> str:
    if question_id == "growth_quality":
        return (
            f"Revenue growth {_percent(values.get('revenue_growth_yoy'))}; "
            f"incremental operating margin {_percent(values.get('incremental_operating_margin'))}; "
            f"top component {_metric_label(str(values.get('top_revenue_component') or 'not extracted'))}"
        )
    if question_id == "profitability_with_scale":
        return (
            f"Gross margin {_percent(values.get('gross_margin'))}; "
            f"operating margin {_percent(values.get('operating_margin'))}; "
            f"net margin {_percent(values.get('net_margin'))}"
        )
    if question_id == "cash_profit_quality":
        return (
            f"CFO / net income {_ratio(values.get('cash_conversion'))}; "
            f"working-capital tailwind / revenue {_percent(values.get('working_capital_cash_tailwind_to_revenue'))}"
        )
    if question_id == "capital_needed_for_growth":
        return (
            f"CapEx / revenue {_percent(values.get('capex_to_revenue'))}; "
            f"FCF margin {_percent(values.get('free_cash_flow_margin'))}; "
            f"incremental ROIC proxy {_percent(values.get('incremental_roic_proxy'))}"
        )
    if question_id == "balance_sheet_resilience":
        return (
            f"Current ratio {_ratio(values.get('current_ratio'))}; "
            f"liabilities / assets {_percent(values.get('liabilities_to_assets'))}; "
            f"restricted cash / cash {_percent(values.get('restricted_cash_to_cash'))}"
        )
    if question_id == "sbc_and_per_share_quality":
        return (
            f"SBC / revenue {_percent(values.get('sbc_to_revenue'))}; "
            f"SBC / CFO {_percent(values.get('sbc_to_operating_cash_flow'))}; "
            f"diluted shares YoY {_percent(values.get('diluted_shares_yoy'))}"
        )
    if question_id == "tax_non_gaap_accounting_quality":
        return (
            f"Effective tax rate {_percent(values.get('effective_tax_rate'))}; "
            f"non-GAAP uplift {_percent(values.get('latest_non_gaap_net_income_uplift'))}"
        )
    return _format_latest_question_values(values)


def _metric_label(metric: str) -> str:
    labels = {
        "transaction_services_revenue": "transaction services revenue",
        "online_marketing_services_revenue": "online marketing services and others",
    }
    return labels.get(metric, metric)


def _warning_label(warning: Any) -> str:
    labels = {
        "Incremental operating margin is negative.": "negative incremental operating margin",
        "Incremental FCF margin is negative.": "negative incremental FCF margin",
        "Incremental operating margin is below latest operating margin.": "incremental margin below current margin",
        "Working-capital source liabilities added more than 5% of revenue to cash-flow tailwind.": "working-capital tailwind > 5% of revenue",
        "Restricted cash exceeds 25% of cash.": "restricted cash > 25% of cash",
        "Investment income/loss is more than 20% of pretax income.": "investment income/loss > 20% of pretax income",
    }
    return labels.get(str(warning), str(warning))
...
def _material_event_summary(scan: dict[str, Any]) -> str:
    ...
    if scan.get("coverage_status"):
        lines.append(f"- Coverage status: {scan.get('coverage_status')}")
    if scan.get("scan_scope_note"):
        lines.append(f"- Scope note: {scan.get('scan_scope_note')}")
    ...
    if not events:
        if scan.get("coverage_status") == "routine_financial_documents_only":
            lines.append(
                "- Routine official financial documents were available and handled by the financial update layer, but no event-style official filing was promoted into the material-event report."
            )
        else:
            lines.append(
                "- No non-core official filing was promoted into the report. This means the scanner did not find a material accounting, auditor, financing, management, governance, dilution, acquisition, impairment, restructuring, or legal/regulatory event within the scanned scope."
            )
        return "\n".join(lines)
    lines.extend(
        [
...
def _operating_kpi_main_summary(analysis: dict[str, Any]) -> str:
    ...
    if latest_by_metric or coverage:
        lines.extend(
            [
                "- Disclosure pattern: KPIs are not all reported every year. Historical marketplace-scale KPIs are separated from current merchant-focused KPIs.",
                "",
                "| KPI / question | Status | Latest official value | Reported period coverage | Read-through |",
                "| --- | --- | ---: | --- | --- |",
            ]
        )
        ...
        for item in coverage:
            ...
            if coverage_metric:
                rendered_metrics.add(str(coverage_metric))
            lines.append(
                "| {label} | {status} | {latest} | {coverage} | {note} |".format(
        ...
        for metric, record in sorted(latest_by_metric.items()):
            if str(metric) in rendered_metrics:
                continue
            lines.append(
                "| {label} | {status} | {latest} | {coverage} | {note} |".format(
                    label=_markdown_cell(record.get("label") or metric),
                    status="latest_record_extracted",
                    latest=_markdown_cell(_format_kpi_value(record)),
                    coverage=_markdown_cell(_kpi_period_coverage(analysis, metric)),
                    note="Latest official KPI record extracted; no separate coverage note generated.",
                )
            )
    if defined_only:
...
def _financial_evidence_open_issue_lines(state: ResearchState) -> list[str]:
    company_id = (state.get("canonical_company") or {}).get("company_id")
    lines: list[str] = []
    if company_id == "tencent":
        lines.extend(
            [
                "Tencent V1 uses Tencent IR financial-report PDFs as the official company source; HKEX official cross-checking is still the next independent official-source layer.",
                "Tencent PDF extraction reads the latest annual report's five-year summary, audited annual statement tables, and the latest mapped interim statement tables.",
                "Tencent official internal cross-checking compares overlapping official PDF facts, such as financial-summary values versus audited statement-table values.",
            ]
        )
    else:
        discovery = state.get("source_discovery", {})
    ...
    lines.append("Recent direct CapEx tags are not always present, so FCF is left blank where official CapEx is unavailable.")
    missing = (state.get("financial_report_pack") or {}).get("missing_facts") or {}
    priority_a_missing = missing.get("priority_a") or []
    if priority_a_missing:
        lines.append("Priority financial facts still missing: " + ", ".join(str(item) for item in priority_a_missing[:8]) + ".")
    return lines
...
def _business_model_source_coverage_linkage_table(state: ResearchState) -> str:
    coverage = (state.get("business_model_findings") or {}).get("source_coverage") or {}
    groups = coverage.get("source_groups") or []
    if not groups:
        return "- Business-model source coverage registry not available."
    lines = [
        f"- Registry: `{_markdown_text(coverage.get('registry_path') or 'not configured')}`",
        f"- Status: {_markdown_text(coverage.get('status') or 'unknown')}",
        "",
        "| Source family | Priority | Collector status | Source target | Source type | Reliability | Locator | Collection method | Limits |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for group in groups:
        for target in group.get("source_targets") or []:
            locator = (
                ...
                or target.get("local_prototype_path")
                or target.get("local_path_glob")
                or ""
            )
            lines.append(
                "| {family} | {priority} | {status} | {target} | {source_type} | {reliability} | {locator} | {method} | {limits} |".format(
                    family=_markdown_cell(group.get("group_id")),
                    priority=_markdown_cell(group.get("priority")),
                    status=_markdown_cell(group.get("collector_status") or group.get("status")),
                    target=_markdown_cell(target.get("source_id") or target.get("name")),
                    source_type=_markdown_cell(target.get("source_type")),
                    reliability=_markdown_cell(target.get("reliability") or group.get("reliability")),
                    locator=_markdown_cell(locator),
                    method=_markdown_cell(target.get("collection_method")),
                    limits=_markdown_cell(target.get("limits")),
                )
            )
    policy = coverage.get("coverage_policy") or []
    if policy:
        lines.extend(["", "### Source Coverage Policy", ""])
        lines.extend(f"- {_markdown_text(item)}" for item in policy)
    return "\n".join(lines)
...
def _report_at_a_glance(state: ResearchState, *, audit_status: str) -> str:
    ...
    fcf_margin = _percent(_safe_divide(latest.get("free_cash_flow"), latest.get("revenue")))
    lines = [
        "| Item | Current read |",
        "| --- | --- |",
        ...
        f"| ROIC / cash conversion / FCF margin | {roic or 'n/a'} / {cash_conversion or 'n/a'} / {fcf_margin or 'n/a'} |",
...
def _official_event_transcript_memo_summary(findings: dict[str, Any]) -> str:
    if not findings:
        return "- Not run."
    lines = [
        f"- Status: {findings.get('status', 'unknown')}",
        f"- Registry: `{findings.get('registry_path') or 'not configured'}`",
        f"- Sources registered: {findings.get('source_count', 0)}",
        f"- Call transcripts collected: {findings.get('call_transcript_count', 0)}",
        f"- Transcript records: {findings.get('transcript_record_count', 0)}",
        f"- Transcript segments: {findings.get('transcript_segment_count', 0)}",
        f"- Evidence items extracted: {findings.get('evidence_item_count', 0)}",
    ]
    question_pack = findings.get("business_model_question_pack") or {}
    if question_pack:
    ...
    theme_summary = findings.get("theme_summary") or {}
    claim_counts = theme_summary.get("claim_counts") or {}
    if claim_counts:
        top_claims = sorted(claim_counts.items(), key=lambda item: int(item[1]), reverse=True)[:6]
        lines.append(
            "- Top transcript evidence themes: "
            + ", ".join(f"{_markdown_text(theme)}={_markdown_text(count)}" for theme, count in top_claims)
        )
    source_results = findings.get("source_results") or []
    if source_results:
        lines.extend(["", "### Earnings-Call / Official-Event Source Readiness", ""])
        for result in source_results[:10]:
            source = _markdown_text(result.get("name") or result.get("source_id") or result.get("period") or "Unnamed source")
            lines.extend(
                [
                    f"#### {source}",
                    "",
                    f"- Adapter: {_markdown_text(result.get('adapter') or 'unknown')}",
                    f"- Period: {_markdown_text(result.get('period') or 'unknown')}",
                    f"- Status: {_markdown_text(result.get('status') or 'unknown')}",
                    f"- Evidence items: {_markdown_text(len(result.get('evidence_items') or []))}",
                    f"- Business-model question results: {_markdown_text(len(result.get('business_model_question_results') or []))}",
                    "",
                ]
            )
        if len(source_results) > 10:
            lines.append(f"- Remaining source results summarized in data linkage: {len(source_results) - 10}")
    return "\n".join(lines)
...
def _business_model_source_coverage_summary(findings: dict[str, Any]) -> str:
    coverage = (findings or {}).get("source_coverage") or {}
    if not coverage:
        return "- Not configured."

    status_counts = coverage.get("status_counts") or {}
    priority_counts = coverage.get("priority_counts") or {}
    lines = [
        f"- Status: {_markdown_text(coverage.get('status', 'unknown'))}",
        f"- Registry: `{_markdown_text(coverage.get('registry_path') or 'not configured')}`",
        (
            f"- Source families: {coverage.get('source_group_count', 0)}; "
            f"targets: {coverage.get('source_target_count', 0)}"
        ),
        "- Collector statuses: "
        + (", ".join(f"{status}: {count}" for status, count in sorted(status_counts.items())) or "none"),
        "- Priorities: "
        + (", ".join(f"{priority}: {count}" for priority, count in sorted(priority_counts.items())) or "none"),
    ]
    if coverage.get("scope"):
        lines.append(f"- Scope: {_markdown_text(coverage.get('scope'))}")

    connected_gaps = coverage.get("top_connected_gaps") or []
    if connected_gaps:
        lines.extend(["", "### Connected Source Families", ""])
        for gap in connected_gaps:
            lines.append(
                "- {name} (`{group_id}`): {priority}, {status}, {targets} targets. Role: {role}".format(
                    name=_markdown_text(gap.get("name") or gap.get("group_id")),
                    group_id=_markdown_text(gap.get("group_id")),
                    priority=_markdown_text(gap.get("priority")),
                    status=_markdown_text(gap.get("collector_status") or gap.get("status")),
                    targets=gap.get("source_target_count", 0),
                    role=_markdown_text(gap.get("evidence_role") or "not specified"),
                )
            )

    local_targets = coverage.get("local_target_status") or []
    if local_targets:
        lines.extend(["", "### Local Prototype Links", ""])
        for target in local_targets:
            status = "exists" if target.get("exists") else "missing"
            lines.append(
                "- {group_id} / {source_id}: {status}; matches {count}; path `{path}`".format(
                    group_id=_markdown_text(target.get("group_id")),
                    source_id=_markdown_text(target.get("source_id")),
                    status=status,
                    count=target.get("match_count", 0),
                    path=_markdown_text(target.get("path")),
                )
            )

    steps = coverage.get("next_collection_steps") or []
    if steps:
        lines.extend(["", "### Next Collection Steps", ""])
        for step in steps:
            outputs = ", ".join(str(item) for item in (step.get("output_expectations") or [])[:4])
            lines.append(
                "- {name}: start from `{locator}`; expected outputs: {outputs}".format(
                    name=_markdown_text(step.get("name") or step.get("group_id")),
                    locator=_markdown_text(step.get("first_locator") or "not specified"),
                    outputs=_markdown_text(outputs or "not specified"),
                )
            )
...
def _business_model_evidence_pack_summary(pack: dict[str, Any], report_path: str | None) -> str:
    if not pack:
        return "- Fixed-question Business Model Evidence Agent has not run yet."

    summary = pack.get("summary") or {}
    questions = pack.get("questions") or []
    p0 = ((pack.get("source_coverage") or {}).get("p0_source_status") or {})
    answered = sum(1 for question in questions if question.get("finding"))
    lines = [
        f"- Evidence report: `{_markdown_text(report_path or 'not written')}`",
        f"- Agent/version: `{_markdown_text(pack.get('agent'))}` / `{_markdown_text(pack.get('version'))}`",
        f"- Fixed questions answered: {answered}/{len(questions)}",
        f"- One-sentence model: {_markdown_text(summary.get('one_sentence_business_model'))}",
        f"- Business model quality: {_markdown_text(summary.get('business_model_quality'))}; confidence: {_markdown_text(summary.get('confidence'))}",
    ]
    if p0:
        lines.append(
            "- P0 source status: "
            + ", ".join(f"{source}: {status}" for source, status in sorted(p0.items()))
        )
    open_issues = pack.get("open_issues") or []
    if open_issues:
        lines.extend(["", "### Fixed-Question Open Issues", ""])
        lines.extend(f"- {_markdown_text(issue)}" for issue in open_issues[:5])
    return "\n".join(lines)
...
This report is produced by the Financial Evidence Agent. It focuses on official financial results, extracted operating KPIs, financial-quality metrics, diagnostic questions, material-event scan boundaries, and source/audit controls.

Excluded from this report: market-price-dependent valuation metrics, valuation commentary, third-party financial databases, and buy/sell conclusions. Those belong to the Valuation / Right Price layer or separate lead-generation checks.
...
{_financial_quality_questions_compact(state, metrics)}
...
## Metric Pack Coverage

{_metric_pack_compact_summary(metrics)}
...
{_bullet_list(_financial_evidence_open_issue_lines(state))}

## Data Linkage

Detailed financial fact IDs, formulas, source documents, official verification records, and audit events are in `{_data_linkage_path(state)}`.
"""
...
## Fixed Business-Model Evidence Questions

{_business_model_evidence_pack_summary(state.get('business_model_evidence_pack', {}), state.get('business_model_evidence_report_path'))}
...
## Business Model Source Coverage

{_business_model_source_coverage_summary(state.get('business_model_findings', {}))}
...
## Earnings Call / Official Event Evidence

{_official_event_transcript_memo_summary(state.get('official_event_transcript_findings', {}))}
...
def _business_model_open_issue_lines(state: ResearchState) -> list[str]:
    ...
    coverage = (state.get("business_model_findings") or {}).get("source_coverage") or {}
    if coverage.get("status") == "business_model_source_coverage_v1":
        lines.append(
            "Priority business-model source families are connected in the registry, but collector snapshots are still needed before using merchant policy, competitor, or investor-material evidence as conclusions."
        )
...
## Right Price / Valuation Snapshot

These price-dependent metrics are produced by the Valuation Agent, not the Financial Evidence Agent.
...
## Financial Formula / Metric Linkage

{_metric_linkage_table(metrics)}

## Official IR Cross-Validation Linkage
...
## Valuation Metric Linkage

{_metric_linkage_table(valuation_metrics)}
...
## Business Model Source Coverage Linkage

{_business_model_source_coverage_linkage_table(state)}
