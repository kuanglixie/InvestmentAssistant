...
def _layer_two_intro(pack: dict[str, Any]) -> str:
    ...
    return (
        ...
        "现金流与流动性披露、VIE / 受限现金 / 税项 / 非 GAAP 调节、季度 6-K / 10-Q、业绩稿，以及 proxy / AGM。"
...
def _management_issue_reviews_section(management_communication_pack: dict[str, Any] | None) -> str:
    ...
    for item in reviews[:7]:
        ...
        lines.extend(
            [
                f"- **{issue}**",
                f"  - 管理层怎么解释：{explanation}",
                f"  - 回答质量：{quality}；与第一层数字的一致性：{consistency}。",
...
def _source_scope_summary(pack: dict[str, Any]) -> str:
    ...
    latest_quarter = trend.get("latest_period_end") or "未识别"
    material_text = (
        f"已扫描 {material_scan.get('scanned_document_count')} 份事件披露，发现 {material_scan.get('material_event_count', 0)} 个重大事项。"
        if material_scan.get("scanned_document_count", 0) > 0
        else "本次没有完整事件披露索引；不能据此断言不存在 6-K / 8-K / proxy 风险。"
    )
    lines = [
        f"- 年报主锚：{annual_line}。",
...
def _core_judgment_section(pack: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *_core_revenue_source_lines(pack, latest, prior, top_component),
        ...
        (
            f"- 利润证据：经营利润率从 {_pct(_margin(prior, 'operating_income'))} 降到 {_pct(_margin(latest, 'operating_income'))}，"
            f"净利率从 {_pct(_margin(prior, 'net_income'))} 降到 {_pct(_margin(latest, 'net_income'))}。"
        ),
        (
            f"- 现金证据："
            f"经营现金流 / 净利润 {_ratio(_safe_divide(latest.get('operating_cash_flow'), latest.get('net_income')))}，"
            f"自由现金流率 {_pct(_margin(latest, 'free_cash_flow'))}，"
            f"营运资本现金顺风 / 收入 {_pct(cash_values.get('working_capital_cash_tailwind_to_revenue'))}。"
        ),
        "",
        "#### C. 资产负债表能不能支撑投入期？",
        "",
        (
            ...
            f"流动比率 {_ratio(_safe_divide(latest.get('current_assets'), latest.get('current_liabilities')))}，"
            f"总负债 / 总资产 {_pct(_safe_divide(latest.get('total_liabilities'), latest.get('total_assets')))}。"
        ),
        (
            f"- 抗压证据：现金 / 总负债 {_ratio(_safe_divide(latest.get('cash'), latest.get('total_liabilities')))}；"
            f"受限现金 / 现金 {_pct(balance_values.get('restricted_cash_to_cash'))}。"
        ),
        (
            f"- 需要保守看的地方：受限现金 / 现金 {_pct(balance_values.get('restricted_cash_to_cash'))}，"
            "所以现金很厚不等于全部现金都能自由用于股东回报或跨境调配；"
            "第二层已把受限现金、VIE 和资金转移限制作为独立证据处理，债务条款和到期结构则作为后续披露/抽取缺口。"
        ),
    ]
    return "\n".join(lines)
...
def _evidence_coverage_table(pack: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            ...
            "已覆盖营运资本、受限现金、VIE 和非 GAAP 的核心证据；债务到期、现金纳税、维护/增长资本开支等细拆仍影响置信度。 |"
        ),
        (
...
def _top_open_items(pack: dict[str, Any]) -> str:
    ...
    if warnings:
        primary = warnings[0]
        lines.append(f"- 利润率压力到底是主动投入，还是结构性竞争压力？当前触发点：{primary}。")
    if missing:
...
def _primary_positive_evidence(pack: dict[str, Any]) -> str:
    ...
    return (
        f"经营现金流 / 净利润 {_ratio(_safe_divide(latest.get('operating_cash_flow'), latest.get('net_income')))}，"
        f"广义现金与短投 {_money(_broad_liquidity(latest))}，流动比率 {_ratio(_safe_divide(latest.get('current_assets'), latest.get('current_liabilities')))}。"
    )
...
def _revenue_structure_summary(pack: dict[str, Any]) -> str:
    ...
    if not result:
        return (
            ...
            "不能判断增长来自产品、地区、分部、take-rate 或交易服务结构变化。"
        )
...
def _working_capital_bridge_summary(pack: dict[str, Any]) -> str:
    ...
    smaller_clause = f"，商家保证金和递延收入等小项合计增加约 {_money(smaller_total)}" if smaller_total else ""
...
def _liquidity_summary(pack: dict[str, Any]) -> str:
    ...
    return "\n".join(lines + [
        ...
        (
            ...
            f"流动比率为 {_ratio(_safe_divide(latest.get('current_assets'), latest.get('current_liabilities')))}。"
            "这支持“能扛投入期”的判断；第二层会把受限现金、VIE、资金转移限制和债务披露单独拆开，"
            "避免把全部账面资金直接当成自由现金。"
        )
    ])
...
def _data_insight_official_review_zh(
    ...
    if asset_growth is not None and revenue_growth is not None and asset_growth > revenue_growth:
        lines.append(
            "- **资产规模扩张快于收入：** 20-F 资产负债表和附注已经能解释主要来源：最新年度总资产同比 "
            f"{_pct(asset_growth)}，高于收入同比 {_pct(revenue_growth)}；广义现金与短投增加 {_money(broad_delta)}，"
            f"约占总资产增量的 {_pct(broad_delta_share)}。"
            f"期末现金、受限现金、短期投资分别为 {_money(latest.get('cash'))}、{_money(latest.get('restricted_cash'))}、"
            f"{_money(latest.get('short_term_investments'))}。"
            "官方文件已经说明资产扩张大部分来自资金和短投沉淀；需要保守处理的是这些资金的可动用性和收益波动。"
        )
...
def _official_question_adjudication_block(
    ...
    question_id = str(question.get("question_id") or "")
    problem = (
        f"{_diagnostic_short_judgment_zh(question)}"
        f"关键证据：{_diagnostic_key_evidence_zh(question)}。"
    )
    if not official_evidence_pack:
        return "\n".join(
            [
    ...
    official_explanation = rendered.get("official_explanation") or "官方文件未提供足够解释。"
    judgment = rendered.get("our_judgment") or (answer.get("our_inference") or {}).get("text") or "暂无判断。"
    ...
    if unknowns:
        unresolved_parts.append("仍未解决：" + "；".join(str(item) for item in unknowns if item))
...
def _watchlist_summary(pack: dict[str, Any]) -> str:
    ...
    lines = [
        "### 下个季度 / 6-K 可直接验证",
        "",
        "- 收入增速是否稳定，交易服务收入和在线营销收入是否延续当前结构。",
...
def _diagnostic_key_evidence_zh(question: dict[str, Any]) -> str:
    ...
    if question_id == "growth_quality":
        component = values.get("top_revenue_component") or "未识别"
        return (
            f"收入同比 {_pct(values.get('revenue_growth_yoy'))}；"
            f"增量经营利润率 {_pct(values.get('incremental_operating_margin'))}；"
            f"最大组件：{_metric_label_zh(str(component))}"
        )
    if question_id == "profitability_with_scale":
        return (
            f"毛利率 {_pct(values.get('gross_margin'))}；"
    ...
    if question_id == "sbc_and_per_share_quality":
        return (
            f"SBC / 收入 {_pct(values.get('sbc_to_revenue'))}；"
            f"SBC / 经营现金流 {_pct(values.get('sbc_to_operating_cash_flow'))}；"
...
def _answer_quality_label_zh(value: Any) -> str:
    labels = {
        ...
        "specific_without_numbers": "具体但缺少量化",
        "directional_only": "只有方向性表述",
        "avoided": "回避或未正面回答",
        "contradicted_by_filings_or_metrics": "与财务数字或 filing 证据冲突",
    }
...
def _interpretation_limit_zh(limit: str) -> str:
    translations = {
        "Source-of-growth attribution uses only official extracted revenue component facts. Missing segment/product/geography/take-rate fields remain evidence gaps.": (
            "增长来源只使用已从官方文件抽取出的收入组件；如果缺少分部、产品、地区或变现率（take-rate）数据，不能把增长来源判断当成完整解释。"
        ),
        ...
        "Working-capital quality is calculated only from extracted official line items. If a company embeds merchant balances inside broader line items, the report must flag that limitation.": (
            "营运资本质量只基于已抽取的官方科目；如果商家余额被放在更宽泛的科目里，现金流判断需要保守。"
        ),
        ...
        "Balance-sheet ratios do not address trapped cash, VIE structure, or capital-control risk.": (
            "资产负债表比率本身不能解决受限现金、VIE 结构或跨境资金限制问题。"
        ),
